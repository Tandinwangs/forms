	...
	<div class="row">
		<div class="container-flexible bnb-border mb-2 p-5 form-description">
			<div class="row">
				...
				<div class="col-sm-3">
					<a href="{{ route('money_gram_claim_forms_path') }}" class="btn btn-sm btn-primary btn-block">Back to MoneyGram Claim Forms</a>
					@if(isset($forms))
			...
			<div class="collapse {{!isset($forms)?'show':''}}" id="expand">
				...
				<form method="GET" action="{{route('search_money_gram_claim_forms_path')}}">
					<div class="row mb-3">
						...
						<div class="col-md-4 mb-3">
							...
							<input type="text" name="MoneyGramReferenceNumber" id="MoneyGramReferenceNumber" class="form-control" placeholder="MoneyGram Reference Number">
						</div>
					</div>
					<div class="row">
				    	...
				    	@foreach($forms as $f)
						    <tr>
						    	<td><small>{{++$i}}</small></td>
						       	...
						       	<td><small>{{$f->moneygram_reference_number}}</small></td>
						       	<td>{{$f->title}}{{$f->name}}</td>
						       	<td>{{$f->mobile_no}}</td>
						       	<td>{{$f->account_number}}</td>
						       	<td>{{$f->sender_title}}{{$f->sender_name}}</td>
						        ...
						        <td class="text-center">
						        	<div class="btn-group">
						        		<a href="{{route('show_money_gram_claim_form_path',[$f->id,'search-show','code'=>$code,'name'=>$name,'moneygram_reference_number'=>$moneygram_reference_number])}}" class="btn btn-primary btn-sm"><small>View</small></a>

...
$('#statusModal').on('show.bs.modal', function (event) {
    ...
    if(action == 'approve'){
        ...
        modal.find('.modal-body #reject').html('<select class="form-control" name="branch" required="required"><option value="">Please Select the Branch</option><option>Corporate Branch (Near Handicraft Bazaar)</option><option>Thimphu Branch (Bhutan Post office)</option><option>Phuntsholing Branch</option><option>Samdrupjongkhar Branch</option><option>Trashigang Branch</option><option>Gelephu Branch</option><option>Paro Branch</option><option>Mongar Branch</option><option>Wangdue Branch</option><option>Bumthang Branch</option><option>Samtse Branch</option><option>Tsirang Branch</option><option>Taba Extension</option><option>Motithang Extension</option><option>Olakha Extension</option><option>Khasadrabchu Extension</option><option>Tala Extension</option><option>Samdrupcholing Extension</option><option>Samdrupjongkhar Extension</option><option>Nganglam Extension</option><option>Kanglung Extension</option><option>Wamrong Extension</option><option>Tashiyangtsi Extension</option><option>Rangjung Extension</option><option>Gomtu Extension</option></select>')
    }
    else if(action == 'change-br'){
        modal.find('.modal-header h5').text('Confirmation for '+name+' Branch Change')
        modal.find('.modal-body p').text('Are you sure you want to change the branch for the submitted '+name+'? Please select the branch from the dropdown menu.')
